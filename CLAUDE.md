...
- Every `productId` referenced in a guide's `outfits[].items[]` must resolve to a real `id` in `js/products.js` — **and to the correct one**. This repo has no schema validation, so a typo silently breaks a shop link with no error anywhere. If you add or edit a `productId` reference, grep `js/products.js` for that exact id and confirm the product name actually matches the item you're describing before moving on.
